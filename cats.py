...
from utils import lower, split, remove_punctuation, lines_from_file, get_key_distances
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def select(x):
        x_list = remove_punctuation(x).lower().split()
        return any(y in x_list for y in topic)
    return select
    # END PROBLEM 2
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    for i in valid_words:
        if i == typed_word:
            return typed_word
        if diff_function(typed_word, i, limit) < minimum:
            minimum = diff_function(typed_word, i, limit)
            returned_word = i

    return typed_word if minimum > limit else returned_word
...
def final_diff(start, goal, limit):
    ...
    if start == "" or goal == "":  # Fill in the condition
        ...
        if start[0] == goal[0]:
            ...
            try: distance = distances[start[0].lower(), goal[0].lower()]
            except: distance = 5
            add = distance + minimum_mewtations(start, goal[1:], limit - distance)
            remove = distance +  minimum_mewtations(start[1:], goal, limit - distance)
            substitute = distance + minimum_mewtations(start[1:], goal[1:], limit - distance)
            return min(add, remove, substitute)
...
def report_progress(sofar, prompt, user_id, send):
    ...
    i = 0
    n = len(sofar)
    while i < n and sofar[i] == prompt[i]:
        i+=1
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    d = [[] for _ in player_indices]
    for i in word_indices:
        minimum = time(game, 0, i)
        minimum_player = 0
        for j in player_indices:
                if time(game, j, i) < minimum:
                    minimum = time(game, j, i)
                    minimum_player = j
        d[minimum_player].append(word_at(game, i))
    return d
